...
import librosa
"""
	TODO: Test with different model and file naming scheme
"""
def ping():
...
def read_sen_labels_from_mdef(fname):
...
def frame2state(fname, phone2state):
...
def genDataset(train_flist, dev_flist, test_flist, n_feats,
	...
	# t.start()
	if context_len != None:
		np.save(outfile_prefix + 'bracketed_train.npy',X_Train)
		np.save(outfile_prefix + 'bracketed_dev.npy',X_Dev)
		np.save(outfile_prefix + 'bracketed_train_labels.npy',Y_Train)
		np.save(outfile_prefix + 'bracketed_dev_labels.npy',Y_Dev)
		if len(X_Test) != 0:
			np.save(outfile_prefix + 'bracketed_test.npy',X_Test)
			np.save(outfile_prefix + 'bracketed_test_labels.npy',Y_Test)
		np.savez(outfile_prefix + 'bracketed_meta.npz',framePos_Train=framePos_Train,
