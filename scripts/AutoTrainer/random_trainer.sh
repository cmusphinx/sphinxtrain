...
DO_TESTS="yes"
DO_TEST_INITIAL="yes"
transcriptionFile="random.transcription"
fileidsFile="random.fileids"
CREATE_SENDUMP="no"
DO_MAP="yes"
DO_MLLR="yes"
SAMPLE_RATE=16000
...
POCKET_SPHINX="no"
transcriptionLine="Error retreiving random sentence!"
...
doReadings() {
#clean our old transcript and fileids
rm $transcriptionFile
rm $fileidsFile
...
do
	...
	clear
	cd recordings
	mv ../output.wav ${i}_random.wav
	echo "<s> $transcriptionLine </s> (${i}_random)" >> $transcriptionFile
	echo "${i}_random" >> $fileidsFile
	cd ../
done
mv recordings/$fileidsFile ./
mv recordings/$transcriptionFile ./
}
...
readSentence() {
	...
	echo " "
	read
	v=$(curl --data "quantity=1&count=10" https://www.randomwordgenerator.org/Random/sentence_generator 2>/dev/null | hxnormalize -x | hxselect -i "p.result" | hxselect -i "b" | hxnormalize -x | lynx -stdin -dump)
	transcriptionLine=${v:6}
	echo $transcriptionLine
	sleep 0.2
